...
//	public bool NearMode = false;
	...
	// Returns whether the player is tilting right
...
//				break;
	...
	void Awake()
	{
		//CalibrationText = GameObject.FindGameObjectWithTag("CalibrationText").GetComponent<Text>();
		//CalibrationText.text = gameObject.GetComponent<Text>().text;
			...
			if(Player1Calibrated)
			{
				...
				foreach(KinectGestures.GestureData gestureData in player1Gestures)
				{
					if (gestureData.complete)
					{
						...
						else if (gestureData.gesture == KinectGestures.Gestures.TiltRight)
						{
							tiltRight = true;
						}
